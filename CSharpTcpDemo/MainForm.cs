...
namespace CSharpTcpDemo
{
    public partial class MainForm : Form
    {
        ...
        {
            PrintLog(string.Format("send to {0}:{1}: StopRobot()", mRobotConnector.IP, mRobotConnector.Port));
            Thread thd = new Thread(() => {
                string ret = mDashboard.StopRobot();
                PrintLog(string.Format("Receive From {0}:{1}: {2}", mRobotConnector.IP, mRobotConnector.Port, ret));
            });
            thd.Start();
        }
        ...
        private void btnJointMovJ_Click(object sender, EventArgs e)
        {
            ...
            PrintLog(string.Format("send to {0}:{1}: JointMovJ({2})", mRobotConnector.IP, mRobotConnector.Port, pt.ToString()));
            Thread thd = new Thread(() => {
                if (mFeedback.IsEnabled())
                {
                    mDashboard.EnableRobot();
        ...
        private void ShowDataResult()
        {
            ...
            if (null != mFeedback.feedbackData.QActual && mFeedback.feedbackData.QActual.Length >= 6)
            {
                ...
                {//第一次填充数据，免得用的时候一个一个输入
                    ...
                    this.textBoxJ5.Text = string.Format("{0:F3}", mFeedback.feedbackData.QActual[4]);
                    this.textBoxJ6.Text = string.Format("{0:F3}", mFeedback.feedbackData.QActual[5]);
                }
            }
